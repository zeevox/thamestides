...
def fetch():
    ...
    request = BeautifulSoup(
        requests.get("https://www.pla.co.uk/hydrographics/ltoverview_table.cfm").content, "html.parser")
    ...
    for row in request.find("tbody").find_all("tr"):
        ...
        hw, lw = children[4].get_text().strip().split(" "), children[5].get_text().strip().split(" ")
        ...
        hw[0] = pytz.utc.localize(datetime.combine(tomorrow.date() if hw_time < get_time.time()
                                                   else get_time.date(), hw_time))
        lw[0] = pytz.utc.localize(datetime.combine(tomorrow.date() if lw_time < get_time.time()
                                                   else get_time.date(), lw_time))
        ...
        try:
            ...
            print(f"ERR: Could not parse gauge data, {tide_gauge_name} gauge is probably offline.")
            output["status"] = 0
        else:
            output["status"] = 1
